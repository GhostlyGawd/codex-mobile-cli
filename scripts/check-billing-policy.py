...
def validate_policy(root: Path) -> list[str]:
    ...
    if not isinstance(policy, dict) or policy.get("schema_version") != 1:
        raise PolicyError("billing policy schema_version must be 1")
    recurring = policy.get("new_recurring_bills", {})
    if (
        recurring.get("maximum") != 1
        or recurring.get("only_allowed_kind") != "fixed_price_monthly_vps"
    ):
        raise PolicyError(
            "billing policy itself must permit exactly one fixed-price monthly VPS"
        )
...
def main() -> int:
    ...
    print(
        "billing policy: PASS (one fixed-price monthly VPS; no metered deployment resource)"
    )
    return 0
