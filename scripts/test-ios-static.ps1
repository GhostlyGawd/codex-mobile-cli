...
Assert-Contains $rootView 'owner PC, WSL, local services, network, or secure ingress' 'The server-unavailable surface must explain the active owner-host availability boundary.'
Assert-Contains $rootView 'This app cannot recreate the server' 'The server-unavailable surface must explain the active-host loss/recovery boundary.'
Assert-Contains (Join-Path $ios 'Sources/Features/Workspaces/NewWorkspaceView.swift') 'active deployment host passes the rootless nested-container isolation spike' 'Nested-container copy must refer to the active host rather than a VPS.'
Assert-Contains (Join-Path $ios 'Sources/Features/Settings/PasskeysSettingsView.swift') 'owner-controlled privileged-console recovery runbook' 'Passkey recovery copy must support the active local host.'
