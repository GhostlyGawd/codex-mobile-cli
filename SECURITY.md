...
The active private-beta host is the owner's Windows PC and D-backed Ubuntu WSL
distribution. Windows, WSL, the local Linux host, and any reviewed ingress are
part of the trusted computing base. The beta is unavailable when the PC, WSL,
services, network, or ingress is offline. The active profile has its own
loop-backed XFS quota boundary under ADR 0026. VPS-specific disk layout,
...
7. Owner-PC and D-backed WSL storage ↔ owner-held recovery copies. A future VPS
   and provider whole-server backup are outside the active beta boundary.
...
| Container escape / host socket exposure | Drop capabilities; no privilege; `no-new-privileges`; seccomp; user namespaces; no engine socket/devices in Coder or workspaces. A dedicated root-owned Podman API is mode `0660` for only `root:coder-provisioner`. The WSL owner profile cannot enforce AppArmor; a future VPS profile still requires it | Runtime/config policy checks, auditd, socket membership checks | Stop runtime/workspaces, rotate provisioner credentials, rebuild host | Shared-kernel zero-days; absence of AppArmor on the selected WSL host; compromise of the provisioner or its private socket is root-equivalent | Host-boundary integration test on the selected WSL kernel, with AppArmor recorded as unavailable rather than passed |
| Cross-workspace leakage | Separate volumes, per-workspace internal and optional egress bridges, Unix users, branches/worktrees, credentials and quotas; no shared writable cache. The owner profile admits only one workload and one persistent quota volume at a time. Each workspace reaches Coder only through its immutable TCP relay; only that relay joins the fixed control uplink, whose host firewall permits one Coder address/port. Privileged Coder API/provisioner credentials never enter workspaces/relays | Canary probes, network-policy validation and audit | Stop affected sessions/relays, rotate scoped Coder credentials and secrets, restore checkpoint | Kernel/provider defect; stale/orphaned runtime state; the unavoidable standard `CODER_AGENT_TOKEN` (`api_key_scope=no_user_data`) lives in the workspace agent process, and same-authority repository code may observe/use it through the relay. It must not confer privileged Coder API or cross-workspace authority | Target-host stale-volume/second-admission denial plus hostile token-scope/relay/route probes; a future multi-workspace profile needs a separate isolation suite |
| Stolen phone/session/replay/deep link | Passkeys; Keychain; short access TTL; rotating refresh; family replay revocation; universal-link validation; no lock-screen approval. Terminal ticket and preview-grant issuance bind to the exact device. Ticket issuance revalidates durable session state inside the bounded per-owner/device revocation gate; ticket redemption/subscriber installation is atomic with the sweep; device/family revocation cancels terminal and preview access and drains admitted terminal mutations and WebSocket writes before return | Replay/device/audit events | Revoke device and sessions through the privileged host admin CLI | Unlocked compromised phone session; ordinary non-authority-creating HTTP work retains normal in-flight semantics | Auth rotation/replay/deep-link tests plus adversarial issue/redeem/write/preview/revoke ordering tests |
...
| Backups/logs/diagnostics/admin credential compromise | Encrypted secret values; root-owned master-key file outside PostgreSQL and database-only dumps; metadata-only logs; redacted deliberate diagnostics. The active beta assumes no provider backup; every owner-controlled whole-host recovery copy must be assessed for whether it also captures the host key. A future provider backup would intentionally capture both ciphertext and that key for consistent recovery | Integrity/restore checks, auditd, secret scans | Rotate keys/tokens, rebuild, restore; after host/full-backup compromise assume encrypted values were readable and rotate or re-enter them | There is no cryptographic separation from compromise of the running host or a whole-host copy; a future provider copy has the same limitation | Restore, diagnostics, secret-scan and compromise-response drills |
| Resource exhaustion on the active local host | Fixed admission pool; continuous admission-to-provider-runtime reservation; durable no-call/ambiguous provider phases; one provider-mutation order; exact-running start/quota barriers; bounded terminal buffers and queues. `owner_pc_beta` permits one 2 CPU / 2 GiB / 512-process workload, one 8–16 GiB persistent quota volume, fixed I/O ceilings, and refuses a start below the 40 GiB XFS free-space boundary | Durable lifecycle/capacity phases, pressure and disk-reserve metrics; storage-volume lease and physical project-ID scan | Resolve provider ambiguity or confirm stop; lifecycle retries fair-share convergence; suspend/stop offending workspace | Ambiguity or provider failure can block the sole slot; heavy workloads slow the PC; a dynamic WSL VHD can consume additional host `D:` space. Dated 2026-07-27 evidence proves the XFS byte/inode quota, singleton refusal, private socket, and cold-start foundation; template-created workload cgroup/I/O/user-mapping and Safe Mode network enforcement remain unproven | Admission/runtime ordering, response-loss, high-water/reconciliation, portable config tests, the dated active-host quota/socket/restart run, and the still-required real Coder workload isolation spike. The historical ten-session profile is deferred |
...
- The active owner-PC profile starts its engine only when the root-owned,
  single-linked 64 GiB image is mounted exactly at `/srv/codex-mobile` as XFS
  with project-quota enforcement, `nodev`, and `nosuid`, and both Windows `D:`
  and guest XFS reserve checks pass. The WSL VHD remains dynamically thin, so
  the guest ceiling is not a host-space preallocation claim.
- The owner firewall, workspace engine, and Compose stack hard-require that
  condition-gated storage service. The first-release source bootstrap may
  install pinned tools and privileged host artifacts only before an active
  release exists; it then refuses working-checkout overwrites, and immutable
  promotion replaces the bootstrap copies with manifest-recorded bytes.
- `owner_pc_beta` permits one persistent quota-bearing named volume in total,
  including stopped or orphaned volumes. This singleton gate is required
  because Podman 4.9.3 can reuse an XFS project ID and mutate an existing
  volume's limit when a second quota volume is created. The immutable volume
  byte quota is 8–16 GiB (12 GiB default); the XFS default project hard and
  soft inode ceiling is 1,048,576 because that Podman release misclassifies its
  named-volume `inodes` option.
- The parent XFS mount and workspace-visible paths remain `nodev`. Only the
  exact root-owned mode-`0700` Podman internal overlay and volume roots receive
  `dev,nosuid` self-binds; neither path enters a workspace. In every profile,
  the private engine socket is root-equivalent and is never mounted into Coder
  or a workspace.
...
- In `owner_pc_beta`, the workload receives exactly 2 CPU, 2 GiB memory with
  no additional swap, 512 processes, 64/32 MiB/s read/write throughput, and
  2,000/1,000 read/write IOPS on the verified backing device. The workload and
  relay each request a non-overlapping 65,536-ID mapping from the dedicated
  `containers:1000000:1048576` subordinate UID/GID pools. Startup rejects an
  I/O-device or mapping-policy mismatch rather than guessing.
- AppArmor is unavailable on the selected WSL host. The owner profile omits
  only that security option and still requires private user namespaces,
  seccomp, `no-new-privileges`, capability drops, cgroups, read-only roots
  where applicable, private networks, and socket/device denial. Future
  fixed-price-VPS acceptance must independently restore and prove AppArmor.
...
- Infrastructure checkpoints select a profile-specific reserve: 24 GiB for
  `owner_pc_beta` and 40 GiB for the deferred VPS profile, plus the configured
  maximum output. They serialize writers, independently check
  producer/compressor status, cap compressed bytes, validate gzip/tar
  integrity, sync, and atomically publish; a capacity refusal is not bypassed.
  Owner-profile checkpoints share the 64 GiB XFS image with the live data and
  are recovery points, not independent disaster backups. Staging artifacts are
  never restore candidates and are removed under the writer lock after an
  interrupted run.
...
- The root-owned master-key file is separate from PostgreSQL and database-only
  exports. The active beta assumes no provider backup; any owner-controlled
  whole-host recovery copy must be assessed for whether it contains both
  encrypted rows and that file. A future provider whole-server backup would
  offer recovery consistency, not cryptographic separation from provider or
...
- Private-beta activation, ingress changes, and restore are deliberate
  owner-approved operator actions; no automatic external or billable mutations
  exist.
