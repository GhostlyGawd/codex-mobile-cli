...
services, network, or ingress is offline. VPS-specific XFS, AppArmor,
provider-backup, reboot, and load controls are deferred rather than inferred.
...
| Resource exhaustion on the active local host | Fixed admission pool; continuous admission-to-provider-runtime reservation; durable no-call/ambiguous provider phases; one provider-mutation order; exact-running start/quota barriers; bounded terminal buffers and queues. The owner-PC profile must set a conservative measured workspace/storage cap before admission is enabled | Durable lifecycle/capacity phases, pressure and disk-reserve metrics | Resolve provider ambiguity or confirm stop; lifecycle retries fair-share convergence; suspend/stop offending workspace | Ambiguity or provider failure can temporarily block starts and underallocate peers; heavy workloads slow all sessions; local filesystem/driver/cgroup enforcement is not proven until the live spike | Admission/runtime ordering, response-loss, high-water/reconciliation, portable config tests, and an active-host load/isolation spike. The historical ten-session XFS profile is deferred |
...
- The active owner-PC profile requires owner-controlled protected storage and a
  measured fail-closed capacity boundary before workspace admission. The
  deferred VPS/XFS profile additionally starts its engine only when
  `/srv/codex-mobile` is the exact XFS mount with `pquota` or `prjquota`. In
  every profile, the private engine socket is root-equivalent and is never
  mounted into Coder or a workspace.
...
- In the deferred VPS/XFS profile, workspace I/O limits bind only to the
  explicit block-device source that `findmnt` reports for
  `/srv/codex-mobile`; that profile rejects a mismatch rather than guessing.
  The dedicated engine applies a fixed creation-time 512-process cgroup
  default because the pinned Terraform provider cannot express `PidsLimit`;
  each active host profile must inspect the template-created container's kernel
  cgroup files.
