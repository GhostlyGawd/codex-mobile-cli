...
| Resource exhaustion on the active local host | Fixed admission pool; continuous admission-to-provider-runtime reservation; durable no-call/ambiguous provider phases; one provider-mutation order; exact-running start/quota barriers; bounded terminal buffers and queues. `owner_pc_beta` permits one 2 CPU / 2 GiB / 512-process workload, one 8–16 GiB persistent quota volume, fixed I/O ceilings, and refuses a start below the 40 GiB XFS free-space boundary | Durable lifecycle/capacity phases, pressure and disk-reserve metrics; storage-volume lease and physical project-ID scan | Resolve provider ambiguity or confirm stop; lifecycle retries fair-share convergence; suspend/stop offending workspace | Ambiguity or provider failure can block the sole slot; heavy workloads slow the PC; a dynamic WSL VHD can consume additional host `D:` space; filesystem/driver/cgroup enforcement is not proven until the live spike | Admission/runtime ordering, response-loss, high-water/reconciliation, portable config tests, and an active-host quota/cgroup/restart spike. The historical ten-session profile is deferred |
