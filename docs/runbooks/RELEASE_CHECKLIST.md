...
- [ ] `scripts/security-audit.sh` passes with the exact pinned tools; govulncheck is clean/dispositioned; every built OCI image has a manifest-bound Syft/Trivy vulnerability/secret/license report for its exact ID, with zero undispositioned findings and no expired/unused disposition.
...
- [ ] Schema-2 immutable release manifest verifies the root-only audit receipt/report tree, exact local image IDs, helper/template/Podman/systemd hashes and installed host artifacts; a Coder activation receipt exists; full health and the bounded disposable smoke gate pass without rebuilding or rescanning.
