...
- **SEV-2:** one-account/device/workspace compromise, repeated auth/preview
  bypass attempt, provider backup failure, sustained capacity exhaustion.
- **SEV-3:** degraded non-security feature with bounded impact.

Open a UTC incident record. Preserve provider, SSH, auditd, Caddy, Coder and
application **metadata-only** logs with hashes and access controls. Never copy
prompts, terminal streams, full commands, source/file contents or secrets into
the record.
...
Treat unauthorized access to a provider whole-server backup as master-key
compromise. The expected backup contains encrypted rows/workspace state and the
root-owned host key together; the offline owner copy is only an availability
copy and does not make that provider artifact cryptographically separate.
