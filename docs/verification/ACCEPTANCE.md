...
As of 2026-07-27, the active owner-PC runtime implementation is present, but
its final consolidated live evidence has not yet been recorded in this file and
is not claimed as passing. Still `GATED`: the one-volume/second-volume-refusal
proof; byte and inherited-inode quotas; template-created workload and relay
cgroups, I/O and distinct user mappings; Coder/EnvBuilder lifecycle and relay
routing; controlled service/WSL restart; and reviewed public HTTPS ingress.
Candidate-image evidence cannot satisfy those checks. The owner profile's XFS
boundary is active; AppArmor, provider backup, VPS reboot, ten-session load, and
the VPS disk layout remain deferred future-host evidence.
