...
- On 2026-07-26, D-backed Ubuntu WSL built and runtime-checked release
  ...
  root-only outside the repository. This proves only those exact developmental
  commit-88 candidates; the final integrated commit must be rebuilt,
  runtime-checked, rescanned, and bound into its release manifest.
...
- D-backed WSL executed the exact developmental commit-88 OCI build, runtime
  ...
  Local production Compose startup was not executed, and the final integrated
  commit still requires its own exact image rebuild, runtime checks, rescan,
  receipt, and manifest binding.
