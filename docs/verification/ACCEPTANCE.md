...
Last updated: 2026-07-27. `PASS` requires executed evidence; a fake at an
...
offline. The active profile has its own 64 GiB loop-backed XFS quota boundary
inside a dynamically sized WSL VHD on `D:`; that evidence must remain distinct
from VPS disk-layout, AppArmor, provider-backup, reboot, ten-session, and load
claims. The local host foundation does not establish public HTTPS, DNS/TLS,
Apple associated domains, a GitHub App, production Coder connectivity, signing,
APNs, or TestFlight availability.
...
| 3 | Workspace | GATED | Terraform 1.14.5 formatting, locked initialization, and validation pass for the Coder template with exact provider/image/helper pins, persistent data, plain/EnvBuilder paths, an immutable create-time `disk_gib` parameter bounded to 8–16 GiB (12 GiB default), and a per-workspace Coder relay/control-uplink contract. The active template profile fixes one workload at 2 CPU / 2 GiB / 512 processes with no additional swap and fixed I/O ceilings; it passes only the exact byte quota to Podman and relies on the host's 1,048,576-inode XFS default. The integrated verifier deterministically cross-built Linux amd64/arm64 workspace helpers and matched both artifacts to the EnvBuilder Dockerfile pins. D-backed WSL built and runtime-checked the exact commit-tagged workspace and EnvBuilder candidates, including the final documentation-bearing candidate. Go/application/lifecycle/Coder/PostgreSQL tests cover durable non-expiring setup-review reconciliation; `suspending`/`deleting` retry states; terminal/preview authority drain under the workspace gate; confirmed provider stop/absence before final persistence; exact cascade finalization; and a fresh PTY on resume. Creation persists only a `private_inputs_pending` non-secret marker while environment/prompt values are encrypted; crash, partial, and legacy ambiguity becomes `private_inputs_recreate_required` before provider work. Autonomy tests prove resume sends the stored mode to provider egress and rewrites managed Codex configuration before returning to running; Full Access has destructive native confirmation. `CODER_WORKSPACE_CONNECTIVITY_CONFIRMED` remains false until a template-created Ubuntu 24.04 workspace proves Coder agent registration/PTY through the relay, Safe Mode no-general-egress behavior, approval success/failure, suspend/resume, Dev Container and persistent-volume behavior on the dedicated private quota runtime. |
| 4 | Isolation | GATED | Static tests verify per-workspace internal/optional-egress networks, data/trusted-helper volumes, immutable non-root/read-only Coder relay, capability/cgroup limits, private user namespaces, non-root interactive users, capability drops, `no-new-privileges`, seccomp, read-only root/helper mounts, no host path/device/runtime socket, and a hostile helper-shadow fixture. The owner profile requires the workload and relay to receive non-overlapping `auto:size=65536` mappings from `containers:1000000:1048576`; only the relay joins the fixed control uplink. AppArmor is unavailable on the selected WSL host and is not claimed. The live suite must prove mapping separation, EnvBuilder boundaries, second-workload and stale-volume refusal, Coder reachability only through the scoped relay, and Safe Mode denial of every other host/control/general-egress destination. A future multi-workspace/VPS profile needs its own cross-workspace and AppArmor evidence. |
| 5 | Host boundary | GATED | Static tests cover the 64 GiB root-owned loop-backed XFS image, `pquota`/`prjquota`, `nodev` parent/workspace paths, two exact root-only Podman-internal `dev,nosuid` self-binds, one persistent quota-volume lease and physical scan, canonical `o=size=<bytes>` with only an exact uppercase `SIZE` mirror accepted, active physical XFS block soft/hard limits equal to those bytes, 1,048,576 inode soft/hard limits, private socket membership, secret files, and billing policy. The surrounding WSL VHD remains dynamically thin on `D:`. Dated 2026-07-27 live evidence at initializer revision `74b44a6` plus cold-start fix revision `c1d6a41` proves the root-owned mode-`0600` image; exact XFS mount options; singleton claim and different-claim rejection; 8 GiB block and 1,048,576-inode soft/hard limits; fail-closed stale-limit handling; write-boundary refusal; inode-limit inheritance on project-ID reuse; two-termination mount persistence; stable global bind count; active services; private `root:coder-provisioner` mode-`0660` socket; exact sub-ID pool; and `10.86.0.1/32` loopback. The row remains `GATED` for real template-created relay/workload cgroup/I/O and distinct-mapping proof, scoped Coder relay routing, Safe Mode network denial, and reviewed public ingress. Every workspace necessarily contains its standard `CODER_AGENT_TOKEN` with `api_key_scope=no_user_data`, and same-authority repository code may observe/use it. AppArmor is unavailable in the selected WSL environment and is not claimed; VPS disk layout, provider-backup, reboot, ten-session, and load evidence is deferred and must not be inferred from local beta behavior. |
...
| 14 | Capacity | GATED (backend PASS) | Public/backend/native disk requests are 8–16 GiB with a 12 GiB default and are sent to Coder only at create. New starts hold admission continuously through the runtime gate and fail closed while another transition may still be live. `owner_pc_beta` fixes the admission maximum at one workload, reserves 2 CPU / 3 GiB / 24 GiB for trusted host/control use, gives the workload exactly 2 CPU / 2 GiB / 512 processes, and refuses a start below 40 GiB free in the 64 GiB XFS image. Podman 4.9.3 requires exactly one persistent quota-bearing named volume because a second can reuse the project ID. The 2026-07-27 live host run proved singleton claim/different-claim refusal, exact 8 GiB block and 1,048,576-inode soft/hard limits, a 1 MiB success, a 9 GiB `ENOSPC` refusal, fail-closed stale-limit handling, correct inode-limit inheritance on project-ID reuse, and cleanup of the proof lease/volume. A real template-created Coder workload must still prove the 2 CPU / 2 GiB / 512-process and I/O cgroups, distinct relay/workload mappings, admission behavior, and capacity refusal through the complete provider flow. The historical ten-session VPS target is deferred. |
| 15 | Recovery | GATED (backend PASS) | Local v2 checkpoint creation/list/hash verification and identity binding, mandatory pre-restore checkpoints, recorded file/deletion restore, atomic replacement, and rollback journaling pass portable tests. On 2026-07-27, an XFS-mount marker survived two WSL terminations and was removed after the proof; this establishes mount persistence only. Live WSL database/workspace recovery and a truthful owner-PC backup boundary remain gated. Provider daily-backup and full-VPS reconstruction evidence is deferred. |
| 16 | Maintenance | GATED (backend PASS) | Immutable release promotion, pre-checkpoint, health-gated rollback, service ordering, and update/incident/credential runbooks pass portable checks. On 2026-07-27, the owner and workspace runtime services returned active after a cold WSL boot, the private socket returned with the required owner/group/mode, and repeated mount-helper execution did not stack global binds. Live local drain, production-stack update, rollback, and full application recovery remain gated. Future VPS reboot/provider-retention drills are deferred. |
...
- On 2026-07-27, the current owner-PC bootstrap working tree passed all 209
  infrastructure tests with 29 expected Windows/POSIX skips, the portable
  release-artifact validator, Ruff lint/format, ShellCheck 0.11.0, POSIX shell
  syntax checks, and the Git whitespace gate. This proves the static
  first-release dependency/order, service-dependency, and documentation
  contracts; it is not evidence that the initializer or product stack ran on
  the live WSL host.
- On 2026-07-27, the exact initializer from revision
  ...
  stale inode limit failed closed and was repaired once; subsequent project-ID
  reuse inherited the required inode limit. A 1 MiB write succeeded, a 9 GiB
  write failed with `ENOSPC`, and no proof lease or volume remained.
- In that same live run, an empty marker on the XFS mount survived two WSL
  terminations. Revision
  [`c1d6a41`](https://github.com/GhostlyGawd/codex-mobile-cli/commit/c1d6a41e85a4ced768307c839fee61e0927f7e01)
  made the active storage mount's parent ID authoritative so a hidden stale
  self-bind cannot satisfy the check, and kept socket finalization bounded
  within the service timeout. After a cold boot, the owner and workspace
  runtime services were active; the socket was `root:coder-provisioner` mode
...
The 2026-07-27 evidence completes the owner-PC storage/quota/singleton,
private-socket, mount-persistence, and cold-service-start foundation checks; it
does not change any full acceptance row to `PASS`. Still `GATED`: a real
template-created Coder relay/workload with cgroup, I/O, and distinct user
mapping evidence; the Coder/EnvBuilder lifecycle; Safe Mode denial of every
other host/control/general-egress route; reviewed public HTTPS ingress and
DNS/TLS/AASA; the owner-created GitHub App; and physical-device/TestFlight
flows. Candidate-image or host-foundation evidence cannot satisfy those checks.
AppArmor is unavailable in the selected WSL environment and is not claimed.
Provider backup, VPS reboot, ten-session load, and the VPS disk layout remain
deferred future-host evidence.
...
- On the 2026-07-25 public-readiness tree, `go work sync`, full `go vet`, and
  ...
  workspace-root module-pattern handling; the module-local equivalent
...
- The 2026-07-25 fresh-clone run used
  ...
  by the pinned Go 1.26 workspace behavior. Full `go vet` and a fresh
...
- The unsigned Xcode/iOS simulator build and automated tests passed in the
  ...
  TestFlight, GitHub App, DNS/TLS, production Coder/EnvBuilder, Safe Mode, and
  public-ingress checks remain `GATED`. The dated owner-PC foundation and cold
  service-start evidence above does not prove full application restart/recovery
  or public availability. Future VPS/provider drills are deferred. The runbooks are executable
