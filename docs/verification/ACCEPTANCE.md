...
## Public repository and CI transition

- The owner approved preserving the original repository and all historical pull
  requests, workflow logs, and commit metadata under the private
  `codex-mobile-cli-private-archive` name.
- The active product tree will be published from a distinct repository with a
  new clean root commit at `GhostlyGawd/codex-mobile-cli`.
...
- Linux and Xcode 26.6 hosted run URLs remain `GATED` until the repository
  transition completes. They must not be reported as passing from workflow
  syntax or local static tests alone.
...
| 18 | Accessibility | GATED | The native source includes accessibility labels/identifiers and app-wide UI tests that launch at the Accessibility XXXL content-size category and keep terminal switching, Git review, and Settings reachable. The asset catalog contains the universal app icon; static policy parses the PNG and proves a non-interlaced 1024×1024, 8-bit truecolor image without alpha/transparency. Xcode compilation, the UI-test run, manual VoiceOver, contrast, touch-target, orientation, and Dynamic Type evidence still require the pinned Mac/device. |
...
- On 2026-07-25, the public-readiness working tree ran
  `pwsh ./scripts/verify.ps1` successfully in 72.648 seconds. All 70
  infrastructure tests passed with six expected Windows skips: five checkpoint
  ...
  policy checks passed. Xcode remained skipped locally and is not a pass until
  the hosted macOS run completes.
- On 2026-07-25, a fresh persistent D-drive clone ran
  `pwsh ./scripts/verify.ps1` successfully in 216.685 seconds. All 68
  infrastructure tests passed, with six expected Windows skips: five
...
- On 2026-07-16, `pwsh ./scripts/verify.ps1` passed on the final tree in 320.2
  seconds with a portable GCC toolchain. Its 66 infrastructure tests passed;
  six POSIX-only execution cases were expectedly skipped on Windows. Coverage
...
`NOT EXECUTED` in this Windows workspace: Docker container image builds, the
dedicated Podman runtime, XFS volume/rootfs project-quota
enforcement/write-past-limit probes, live PID/I/O cgroups on a
template-created workspace, EnvBuilder/Dev Container runtime, Coder workspace
lifecycle, host hardening, ten-session load, restore/rollback/reboot drills,
public DNS/TLS, and provider backup restoration. Those checks remain `GATED`;
their results must not be inferred from static evidence.
...
- On the 2026-07-25 public-readiness tree, `go work sync`, full `go vet`, and
  ...
  because this Windows host has no CGO compiler; the public Linux workflow is
  the required executed race gate.
...
- The final integrated verifier passed iOS static policy, supply-chain
  reproducibility, and release-artifact validation. The exact-tree
  source-security audit then passed with Syft 1.46.0, Trivy
...
- The retired private Windows workflow completed a bounded trusted smoke before
  ...
  only in the private historical archive. That evidence does not satisfy Linux
  race, Xcode, container, or public-workflow acceptance.
...
- Xcode/iOS build, signing, App registration, APNs, TestFlight, GitHub App,
  DNS/TLS, VPS deployment and every provider/restore/load/reboot drill remain
  `GATED`. The runbooks are executable guidance and safety policy, not evidence
  those external actions occurred.
- `pwsh ./scripts/test-ios-static.ps1` passed on Windows. It covers the universal
  ...
  cache-only heuristic redaction. The required Xcode 26.6/XcodeGen 2.45.4
  generation and `xcodebuild` test on the iPhone 17 Pro simulator are `GATED`
  and `NOT EXECUTED` on Windows; they are not reported as passing.
