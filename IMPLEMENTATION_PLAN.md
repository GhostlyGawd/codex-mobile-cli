...
| --- | --- | --- |
| GitHub repository | Clean-history public publication is owner-approved; the current repository remains private until readiness checks and runner retirement complete | Preserve the original as `codex-mobile-cli-private-archive`, then publish the verified tree from a distinct repository at the original product URL |
| Go | 1.26.5 installed and verified | Backend build/test available |
| Windows self-hosted runner | Historical trusted smoke validation passed; the public-safe workflow removes every route to it | Unregister and remove only this repository's listener and launcher before creating the public repository |
...
| Swift/Xcode | Local Xcode is unavailable on Windows; public CI pins standard `macos-26`, Xcode 26.6, checksum-verified XcodeGen 2.45.4, and the iPhone 17 Pro simulator | Hosted compile/simulator evidence becomes available after publication; signing, physical devices, APNs, and TestFlight remain owner-gated |
...
| 1. Reproducible skeleton | locally verified | Milestone 0 boundaries | Full Go vet/race/coverage, migrations, deterministic Linux cross-builds, Compose/static infrastructure validation, iOS static contract policy, and public standard-runner CI policy pass; live hosted evidence is pending publication |
...
| 6. Files/Git/review | owner-gated | Production Linux workspace filesystem | Linux fd-relative no-follow file/search/save, exact displaced-content ETag CAS, bounded Git, and recoverable checkpoint/discard contracts pass locally; Xcode, live contention, and target-filesystem proof remain gated |
| 7. Previews/secrets/offline | owner-gated | Domain/TLS for live preview E2E and Mac/device for native runtime tests | Preview revocation, live grant/revoke tmpfs sync, pre-replay terminal redaction, encrypted read-only cache/drafts/history, and attachment cache exclusion pass portable/static tests; live platforms remain gated |
| 8. Operations/hardening | owner-gated | Selected VPS for measured load/restore | Supply-chain reproducibility, exact-tree source-security scanning, release-artifact validation, private quota-runtime policy, and runbooks are locally exercised; built OCI image scans and XFS quota/restore/maintenance/load drills remain VPS-gated |
| 9. Product polish/release | owner-gated | Apple team, APNs key, physical devices, TestFlight authorization | Hosted Xcode simulator tests plus VoiceOver/Dynamic Type device evidence, signed archive, and release checklist |
...
- [x] Add public-only standard Linux/macOS workflows, remove the self-hosted
  ...
  policy tests.
- [ ] Merge public-readiness changes while the original repository is private.
- [ ] Unregister and remove only the `codex-mobile-cli` owner-PC runner.
- [ ] Rename the original repository to the private historical archive and
  publish the verified tree from a distinct public repository.
- [ ] Require successful hosted Linux and Xcode simulator checks and record
  their URLs before closing the publication milestone.
...
- [x] Run the final source-security scanner against the frozen publication tree.
...
the integrated verifier completed in 72.648 seconds and passed 70
infrastructure tests with six expected Windows skips. The literal root
...
executed race result remains assigned to the public Linux workflow. Xcode 26.6
remains assigned to the hosted macOS workflow.
...
The same final-tree verifier passed 66 infrastructure tests with six expected
...
Xcode 26.6/XcodeGen 2.45.4 testing on the iPhone 17 Pro simulator is `GATED`
and `NOT EXECUTED` on Windows, as are Docker/Podman built-image inspection and
all provider/VPS, Apple, domain, and credentialed end-to-end scenarios.

The exact-tree source-security audit completed with Syft
